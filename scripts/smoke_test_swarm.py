"""Smoke test: 10 steps × N=2 of the swarm pipeline.

Catches: vmap-incompat in the swarm consensus path, shape mismatches in the
probe-batch broadcast, missing FedHistory fields. Should finish in <60s.
"""
import sys
...
# tiny shards, just to spin the loop
...
# probe batch: 16 fresh train samples
...
def make_model():
    m = AutoModelForMaskedLM.from_pretrained("roberta-base", attn_implementation="eager").to(DEVICE)
    m.eval()
    return m
...
print("Loading model...")
_ = make_model(); del _
print("Running 10-step swarm smoke test (N=2, K=5)...")
...
hist = train_fedavg_mezo(
    ...
    log_every=2,
    seed=0,
    swarm_config=SwarmConfig(eval_batch=probe_batch, alpha=0.5, beta=2.0),
)

print(f"\nfinal val acc: {hist.eval_acc[-1]:.4f}")
print(f"swarm rounds:  {len(hist.swarm_weights)}")
print(f"swarm weights per round:")
for i, w in enumerate(hist.swarm_weights):
    print(f"  round {i+1}: {[f'{x:.3f}' for x in w]}  (sum={sum(w):.4f})")
print(f"swarm eval losses per round:")
for i, l in enumerate(hist.swarm_eval_losses):
