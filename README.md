...
3. **Репутационная модуляция `W`** — гиперпараметр `β` задаёт непрерывный спектр
   «кооперация ↔ отбор»; на гладком LM-подобном ландшафте найдено рабочее окно
   `β ∈ [0.05, 0.5]`, где отбор ускоряет ранний descent на ≈30% против
   FedAvg-MeZO, а при больших `β` происходит замедление вплоть до
   почти-каскада.
...
| **H3** | Репутационная `W` имеет оптимум `β` внутри спектра: умеренный отбор ускоряет сходимость, жёсткий — почти каскадирует | E3: 50 запусков × 7 `β` на QuadraticWithWells M=10 | loss(step=50): `−1.50` (FedAvg) → **`−1.98`** (`β=0.1`) → `+1.13` (`β=50`); финальный точно совпадает у всех (`f=−3.0`) | ✅ подтверждено |
...
**Day 4 — репутационная W.** Перенос результата E3 (рабочее окно по `β`,
ускоряющее ранний descent, и почти-каскад при больших `β`) с синтетического
QuadraticWithWells M=10 на лосс RoBERTa+SST-2 (тот же non-IID Dirichlet split,
N=8, K=100). Тестируется sweep `β ∈ {0, 1, 10, 100}`: `β=0` совпадает с
FedAvg, при больших `β` ожидается замедление сходимости.
