...
- Спектр по `β`: `β=0` → симметричный FedAvg (модель Де Гроота, ровно `W = (1/N)·J`); `β ∈ [1, 10]` — рабочее окно с измеримым выигрышем; `β → ∞` — информационный каскад в локальный минимум.
...
- `β` (репутационная модуляция, см. `src/reputation.py`) — рабочее окно `[1, 10]` по E3, `β=0` совпадает с FedAvg, `β=100` каскадирует
...
- Перенос рабочего окна `β ∈ [1, 10]` из E3 (multi-well) на реальный лосс RoBERTa+SST-2.
...
- **`β`** в репутационной W задаёт спектр «кооперация ↔ отбор»; рабочее окно `[1, 10]`
