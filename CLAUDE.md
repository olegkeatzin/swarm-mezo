...
Результаты сохраняются в `outputs/` (коммитятся в репозиторий — см. `.gitignore`). Скрипты идемпотентны — уже выполненные конфигурации пропускаются.
...
└── outputs/                    # JSON с результатами (коммитятся)
```
